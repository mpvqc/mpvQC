...
def _parse_comments(data: dict) -> tuple[Comment, ...]:
    match data.get("comments"):
        case list(comments):
            return tuple(_parse_comment(comment) for comment in comments)
        case other:
            msg = f"Expected 'comments' to be a list, got: {type(other).__name__}"
            raise ValueError(msg)


def _parse_comment(entry: object) -> Comment:
    if isinstance(entry, dict):
        ...
        if (
            ...
            return Comment(
                time=TimeFormatterService.parse_subsecond_string_to_milliseconds(time),
