...
_STYLE_MESSAGES = frozenset({_WM_STYLECHANGING, _WM_STYLECHANGED})
...
def _route_popup(probe: MessageProbe, hwnd: int) -> MessageRoute:
    # SetWindowLong sends the style messages back through this filter
    # synchronously, so answering one would call SetWindowLong again, recursing
    # until win32k's nested-message limit.
    if probe.message_id() in _STYLE_MESSAGES:
        return PASS_THROUGH
