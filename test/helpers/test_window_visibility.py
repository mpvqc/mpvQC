...
from mpvqc.helpers.window_visibility import MpvqcWindowVisibilityHandler
...
def configure_inject(window_properties_service_mock, window_mock):
    with patch("mpvqc.helpers.window_visibility.QGuiApplication.topLevelWindows", return_value=[window_mock]):
...
def handler(window_properties_service_mock):
    with patch("mpvqc.helpers.window_visibility.QGuiApplication.topLevelWindows") as mock_windows:
        window = MagicMock()
