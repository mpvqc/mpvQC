...
from mpvqc.services.document_exporter import DocumentExportService
from mpvqc.services.document_importer import DocumentImporterService
from mpvqc.services.import_export_wiring import ImportState
...
def configure_for_open(mock_doc_importer, mock_sub_importer, settings_service):
    def setup(document_videos, subtitle_videos, doc_setting, sub_setting):
        ...
        mock_doc_importer.read.return_value = DocumentImporterService.DocumentImportResult(
            valid_documents=[], invalid_documents=[], existing_videos=document_videos, comments=[]
        )
...
@pytest.fixture
def service():
    return ImportExportWiringService()
...
def test_open_synchronous(
    ...
    spy = make_spy(service.import_about_to_finish)

    service.open(documents=[], videos=test_case.dropped_videos, subtitles=[])

    assert spy.count() == 1
    assert spy.at(0, 0) == test_case.expected_video
...
def test_open_with_user_interaction(
    ...
    finish_spy = make_spy(service.import_about_to_finish)
    ...
    assert finish_spy.at(0, 0) == test_case.expected_video


def test_continue_with_import_opens_video_when_selected(service, mock_player, make_state):
    state = make_state(selected_video=Path("/path/to/video.mp4"))

    service._continue_with_import(state)

    mock_player.open_video.assert_called_once_with("/path/to/video.mp4")


def test_continue_with_import_opens_subtitles_when_present(service, mock_player, type_mapper, make_state):
    state = make_state(subtitles=[Path("/path/to/sub.srt")])

    service._continue_with_import(state)

    mock_player.open_subtitles.assert_called_once_with(subtitles=["/path/to/sub.srt"])


def test_continue_with_import_imports_documents_when_video_present(service, mock_state, make_state):
    docs = [Path("doc.txt")]
    video = Path("/video.mp4")
    state = make_state(selected_video=video, valid_documents=docs)

    service._continue_with_import(state)

    mock_state.import_documents.assert_called_once_with(documents=docs, video=video)
