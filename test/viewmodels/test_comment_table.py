...
def make_view_model():
    def _make(comments: list[Comment]):
        ...
        vm._comments.import_comments(tuple(comments))
        return vm
...
def test_on_view_action_quick_selection(make_view_model, make_spy):
    vm = make_view_model(comments=[])
    spy = make_spy(vm.quickSelectionRequested)

    vm._on_view_action(QuickSelection(row=3))

    assert spy.count() == 1
    assert spy.at(0, 0) == 3


def test_on_view_action_animated_selection(make_view_model, make_spy):
    vm = make_view_model(comments=[])
    spy = make_spy(vm.selectionRequested)

    vm._on_view_action(AnimatedSelection(row=7))

    assert spy.count() == 1
    assert spy.at(0, 0) == 7


def test_on_view_action_quick_selection_and_edit(make_view_model, make_spy):
    vm = make_view_model(
        comments=[
    ...
    vm._on_view_action(QuickSelectionAndEdit(row=1))
...
def test_on_view_action_no_view_action(make_view_model, make_spy):
    vm = make_view_model(comments=[])
    quick_spy = make_spy(vm.quickSelectionRequested)
    selection_spy = make_spy(vm.selectionRequested)

    vm._on_view_action(NoViewAction())
