...
services, models, view models, and QML enums the area owns. A feature's domain holds only the area's business rules;
pure presentation state lives in the view model role, a plain Python module beside the QObject adapters it feeds. It
also owns the settings keys its area means, reading the file through the shared settings file service. The application
