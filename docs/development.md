...
how the codebase fits together, see [architecture.md](architecture.md).
...
bundle. Configure your IDE to run it before launching the app.
...
so the test runners load an up-to-date resource bundle. When iterating on test code only, `just test-qml` and
`just test-python` run directly without recompiling.
