...
MpvqcInputTextField {
    ...
    TestCase {
        ...
        SignalSpy { id: acceptedSpy; target: objectUnderTest; signalName: 'acceptedInput' }
        SignalSpy { id: stoppedSpy; target: objectUnderTest; signalName: 'editingStopped' }
        ...
        function test_validAccept_data() {
            return [
                { tag: 'enter', exec: () => keyClick(Qt.Key_Enter), },
                { tag: 'from-outside', exec: () => objectUnderTest.accepted(), },
            ]
        }
        ...
        function test_validAccept(data) {
            ...
            o.text = o.goodWord
            verify(o.hasText);   compare(o.text, o.goodWord)
            verify(!o.hasError); compare(o.error, '')
            verify(o.focus)

            data.exec()
            compare(acceptedSpy.count, 1)
            compare(stoppedSpy.count, 1)
            verify(!o.hasText);  compare(o.text, '')
            verify(!o.hasError); compare(o.error, '')
            verify(!o.focus)
        }
        ...
        function test_validReject_data() {
            return [
                { tag: 'escape', exec: () => keyClick(Qt.Key_Escape), },
                { tag: 'from-outside', exec: () => objectUnderTest.rejected(), },
            ]
        }
        ...
        function test_invalidAccept(data) {
            ...
            o.text = o.badWord
            verify(o.hasText);   compare(o.text, o.badWord)
            verify(o.hasError);  compare(o.error, o.exampleError)
            verify(o.focus)
