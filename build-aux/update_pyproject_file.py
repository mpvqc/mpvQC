...
class ProjectFileUpdater:
    ...
    _EXTENSIONS_IGNORED = frozenset({".pyc", ".qm", ".qmlc", ".rcc"})
    ...
    def sorted_files_as_str(self) -> list[str]:
        return [p.as_posix() for p in sorted(self._files, key=lambda p: str(p).lower())]
...
def determine_line_to_overwrite(lines: list[str]) -> tuple[int, int]:
    ...
    if not in_section:
        msg = 'Could not find "[tool.pyside6-project]" in pyproject.toml'
        raise KeyError(msg)
    if files_start is None:
        msg = 'Could not find "files" in "[tool.pyside6-project]" table in pyproject.toml'
        raise KeyError(msg)

    msg = 'Could not update "files" in pyproject.toml'
